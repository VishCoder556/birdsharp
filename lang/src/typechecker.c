...
bool are_equal(AST_TypeInfo *left, AST_TypeInfo *right){
    if (left->kind == right->kind || left->kind == KIND_CONST || right->kind == KIND_CONST || left->kind == KIND_AUTO || right->kind == KIND_AUTO) {
        return true;
    }else {
        return false;
    }
    ...
    if (strcmp(l, r)) {
        if (left->kind == KIND_CONST || left->kind == KIND_AUTO){
            *left = *right;
        }else if (right->kind == KIND_CONST || right->kind == KIND_AUTO){
            *right = *left;
        }else {
            return false;
        }
    ...
    if (ast.type == AST_RET){
        ...
        for (int i=0; i<externlen; i++){
            if (strcmp(externs[i], ast.data.funcall.name) == 0){
                return (AST_TypeInfo){.type = "int", .kind = KIND_I32, .ptrnum = 0};
            };
        }
        ...
        if (!are_equal(&left, &right)) {
            if (!((left.ptrnum > 0 && right.ptrnum == 0) || (right.ptrnum > 0 && left.ptrnum == 0) && (ast.type == AST_PLUS || ast.type == AST_SUB))){
                char string[100];
        ...
        return (AST_TypeInfo){"", KIND_I32, 0};
        ...
        }else if(typeinfo.kind == KIND_ARRAY){
...
int typechecker_eat_ast(Typechecker *typechecker){
    ...
    AST *ast = typechecker->iter;
    typechecker_eat(typechecker, ast);
    typechecker->iter = typechecker->iter->next;
    typechecker->cur++;
    return 0;
};
